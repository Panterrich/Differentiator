...
void Tree_clone(struct Tree* new_tree, struct Tree* old_tree)
{
    Tree_null_check(new_tree);
    Tree_null_check(old_tree);

    new_tree->name_equation = old_tree->name_equation;
    new_tree->name_table    = old_tree->name_table;
...
void Node_validator(struct Tree* tree, struct Node* current_node, size_t* count)
{
    ...
    if (fabs(current_node->type - 2.5) > 2)
    {
        tree->error = TREE_WRONG_NODE_TYPE;
        return;
    }

    if (((current_node->type == NUMBER) || (current_node->type == OPERATION) || 
         (current_node->type == FUNC)) && (isnan(current_node->value)))
    {
        tree->error = TREE_WRONG_NODE_VALUE;
        return;
    }

    if (((current_node->type == VAR) || (current_node->type == FUNC)) && (current_node->str == nullptr))
    {
        tree->error = TREE_WRONG_NODE_STR;
        return;
    }

    if ((current_node->type == FUNC) && (current_node->left != nullptr))
    {
        tree->error = TREE_WRONG_FUNC_LEFT_CHILD;
        return;
    }

    if ((current_node->type == VAR) && ((current_node->left != nullptr) || (current_node->right != nullptr)))
    {
        tree->error = TREE_WRONG_VAR_CHILD;
        return;
    }

    if ((current_node->type == NUMBER) && ((current_node->left != nullptr) || (current_node->right != nullptr)))
    {
...
void Tree_dump(struct Tree* tree, FILE* log)
{
    ...
    fprintf(log, "Tree (ERROR #%d: %s [%p] \"%s\" Base: \"%s\" \n"
                  "{\n"
                  "\tsize = %u\n" 
                  "\tdata[%p]\n",  
...
void Print_node(struct Tree* tree, struct Node* current_node, size_t* count, FILE* log)
{
    ...
    fprintf(log, "\t\tNode %d [%p]: Type: %d; Value: %lg; Str: %s; Left: [%p]; Right: [%p];\n",
...
{   
    ...
    {
        fprintf(text, "\t\"%p\" [shape = box, label = \"%c\" style = filled, color = black, fillcolor = lightskyblue]\n", current_node, (char)current_node->value);
    }
...
{   
    ...
    switch (tree->error)
    {
        ...
        case_of_switch(TREE_WRONG_NAME_DATA_EQUATION)
